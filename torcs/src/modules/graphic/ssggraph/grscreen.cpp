...
void cGrScreen::camDraw(tSituation *s)
{
    ...
    if (drawIndicator == 2){
        ...
        const tdble dist = 1.0;
        tdble CosA = cos(curCar->_yaw + PI);
        tdble SinA = sin(curCar->_yaw + PI);
        tdble x = curCar->_pos_X + dist * CosA;
        tdble y = curCar->_pos_Y + dist * SinA;

        tdble offset_x = cos(curCar->_yaw + PI + angle) * 2.0;
        tdble offset_y = sin(curCar->_yaw + PI + angle) * 2.0;

        curCam->eye[0] = x + offset_x;
        curCam->eye[1] = y + offset_y;
        curCam->eye[2] = curCar->_pos_Z + 0.5;
        curCam->center[0] = curCar->_pos_X + offset_x;
        curCam->center[1] = curCar->_pos_Y + offset_y;
        curCam->center[2] = curCar->_pos_Z + 0.5;

        curCam->speed[0] = curCar->pub.DynGCg.vel.x;
        curCam->speed[1] = curCar->pub.DynGCg.vel.y;
        curCam->speed[2] = curCar->pub.DynGCg.vel.z;
