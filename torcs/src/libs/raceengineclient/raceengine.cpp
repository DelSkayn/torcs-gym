...
extern sem_t* sem_written;
extern sem_t* sem_read;
extern uint8_t* pdata;
extern uint8_t* pdata_left;
extern uint8_t* pdata_right;
extern uint8_t* pdata_between;
...
ReOneStep(double deltaTimeIncrement)
{
    ...
    if (zombieID < 0){
		for (i = 0; i < s->_ncars; i++){
			if (strncmp(s->cars[i]->_name, "ficos", 5) == 0){
                zombieID = i;
            }
        }
		if (zombieID < 0){
	...
	switch (ReInfo->_displayMode) {
        ...
        case RM_DISP_MODE_TRAIN:
            ...
            if (ReInfo->s->currentTime >= 0){
                drawIndicator = 0;
                ReInfo->_reGraphicItf.refresh(ReInfo->s);
                glReadPixels(0,0, IMAGE_WIDTH,IMAGE_HEIGHT,GL_RGB,GL_UNSIGNED_BYTE, (GLvoid*)pdata_between);
            }
            ...
            if (ReInfo->s->currentTime >= 0){
                drawIndicator = 1;
                ReInfo->_reGraphicItf.refresh(ReInfo->s);
                glReadPixels(0,0, IMAGE_WIDTH,IMAGE_HEIGHT,GL_RGB,GL_UNSIGNED_BYTE, (GLvoid*)pdata_left);

                drawIndicator = 2;
                ReInfo->_reGraphicItf.refresh(ReInfo->s);
                glReadPixels(0,0, IMAGE_WIDTH,IMAGE_HEIGHT,GL_RGB,GL_UNSIGNED_BYTE, (GLvoid*)pdata_right);
